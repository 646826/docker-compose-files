...
| `uptime` | Community | `uptime` | Uptime Kuma `2.4.0-slim` | `make uptime` | `uptime.${BASE_DOMAIN}` | `uptime_kuma_data` | protected by selected Traefik middleware | none | `make check-community-runtime` |
...
The Uptime Kuma module intentionally uses the pinned slim image. It keeps the ordinary SQLite-backed deployment and standard monitor types while excluding embedded MariaDB and embedded Chromium. Operators who need Browser Engine monitors or embedded MariaDB can select the full `2.4.0` image in a local override after reviewing the larger dependency and vulnerability surface.
